...
//         if (employeeId) {
//           const formattedDate = deliveryDate.toISOString().split('T')[0]; // Định dạng ngày
...
//               order.delivery_date.startsWith(formattedDate) // Chỉ hiển thị đơn hàng có ngày giao đúng
//             );
...
//         Alert.alert('Lỗi', 'Không có đơn hàng tồn tại');
//       } finally {
//         setLoading(false);
//       }
//     };
    ...
    const fetchOrders = async () => {
      ...
      try {
        ...
        if (employeeId) {
          ...
          if (response.data.success) {
            // Lọc đơn hàng chỉ lấy phần ngày 'YYYY-MM-DD' từ 'delivery_date'
            const filteredOrders = response.data.data.filter(order => 
              order.delivery_date.split(' ')[0] === formattedDate // So sánh phần ngày của 'delivery_date'
            );
